...
app.post("/auth", (req, res) => {
  ...
  // check the user name
  var identity = contacts.find(pass => pass.name === name);
  if (identity == undefined){
    res.send(`<h1> Name not found </h1>`);
    res.redirect(401, "/login/");
  };


  // check the user name and password
  if(password == identity.password && identity.role == "editor") {
  ...
  res.send(`<h1> Password or role does not match </h1>`);
  res.redirect(401, "/login/");
}
